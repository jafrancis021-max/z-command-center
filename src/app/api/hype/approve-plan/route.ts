...
import { getLatestPlanByWallet, approvePlan,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (hasPlanId && typeof plan_id === 'string') {
      ...
      if (!latest) {
        return NextResponse.json(
          { ok: false, error: 'No plan found for this wallet. Create a plan first.' },
          { status: 404 },
        )
      }
    ...
    await approvePlan(resolvedPlanId, normalizedWallet)
    ...
    console.error('[api/hype/approve-plan]', message)
